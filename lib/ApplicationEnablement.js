...
class ApplicationEnablement {
   ...
   * AE.deleteBaseUrl()
   */
  setBaseUrl(microservice, doCheck = true) {
    ...
    const MICROSERVICES = [
      ...
      'composite-things', 'composite-things-odata', 'composite-events-odata', 'advancedlist-thing-sap',
      'cs-hierarchy-meta-sap', 'events-sap', 'config-package-sap', 'bpanalytics-event-sap',
      'events-aggregate-sap', 'appiot-fs', 'tm-data-mapping', 'details-thing-sap']; // a list of all available microservices

    // check if the passed microservice is available
    if (doCheck && !MICROSERVICES.includes(microservice)) {
